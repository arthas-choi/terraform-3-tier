...
resource "aws_subnet" "tier-public-subnet-2" {
  ...
  availability_zone = "${length(data.aws_availability_zones.available.names)>3 ? data.aws_availability_zones.available.names[2] : data.aws_availability_zones.available.names[1]}"
  map_public_ip_on_launch=true
  tags = {
